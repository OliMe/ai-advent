...
**Поиск по коду (опц., тумблер `SUPPORT_CODE_SEARCH=1`):** для технических issues от разработчиков бот
может смотреть и в КОД репозитория. Подключает `git-mcp`, детерминированно (по образцу `/ask` Дня 31):
модель называет 1–3 grep-шаблона по реальному списку файлов → форс-`git_grep` → чтение файлов →
фрагменты кода становятся такими же цитируемыми доказательствами, как FAQ (тот же цитатный гейт;
`code-evidence.ts` поверх `llm-cli/code-search.ts` + `grounding.toEvidenceChunks`). По умолчанию выкл:
поддержка пользователей отвечает по FAQ (код им не нужен). **Требуется модель посильнее** (выбор
шаблонов и синтез по коду — на qwen 3B не тянет, деградирует к FAQ; на 7B/облаке работает) и
`SUPPORT_REPO_ROOT`. Сбой код-поиска НЕ роняет ответ — мягкая деградация к ответу по FAQ. В CI нужен
`npm --prefix .ai-advent/git-mcp install` в workflow.
...
| `SUPPORT_CODE_SEARCH=1` | Поиск по КОДУ репозитория (подключает `git-mcp`); нужен `SUPPORT_REPO_ROOT`. Выкл по умолчанию |
