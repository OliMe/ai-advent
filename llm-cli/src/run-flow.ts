...
/**
 * Потолок раундов инструментов для аналитика требований — 20 (по просьбе пользователя): хватает
 * изучить проект (несколько чтений) и задать вопрос. Безопасно даже при интерактивности, т.к.
 * `dedupToolResult` не даёт истории раздуваться на повторных чтениях (прежняя причина «зависания» —
 * бесконечное перечитывание — устранена дедупом; offload-L3, гнавший этот цикл, откачен).
 */
const CLARIFIER_MAX_TOOL_ROUNDS = 20;
...
const CLARIFIER_SYSTEM =
  ...
  'ответ по умолчанию. Если в контексте дана карточка ПРОЕКТА (корень, документация, команды, ' +
  'ветка) — репозиторий, его файлы и способ проверки УЖЕ известны, а изменения вносятся в файлы ' +
  'проекта автоматически. НЕ спрашивай, где проект, какой у него корень или рабочая директория, ' +
  'есть ли доступ к файлам и куда/как применять изменения — это уже определено системой. Уточняй ' +
  'только СУТЬ задачи (что именно и в каком объёме менять, критерии готовности), а не окружение. ' +
  'У тебя есть инструменты чтения проекта (файлы/структура/поиск по коду). СНАЧАЛА при необходимости ' +
  'изучи релевантные файлы (README, package.json, конфиги, затронутый задачей код), а ПОТОМ ' +
  'спрашивай — так вопросы будут предметными и опирающимися на реальное состояние репозитория. То, ' +
  'что можно УЗНАТЬ из репозитория (версии зависимостей, фреймворк, структуру, текущее поведение), ' +
  'выясняй САМ, а НЕ переспрашивай. Задавай ТОЛЬКО действительно важные вопросы: настоящие развилки, ' +
  'которые МЕНЯЮТ решение, и которые может решить лишь пользователь (его намерение, объём, ' +
  'приоритеты, значимые крайние случаи, спорные компромиссы). НЕ задавай очевидных, малозначимых или ' +
  'общих вопросов ради галочки, не выпытывай мелочи, выводимые из задачи и кода. Если задача и ' +
  'репозиторий уже достаточно определяют работу — заверши БЕЗ вопросов. Лучше один точный вопрос, чем ' +
  'три поверхностных. Не используй инструменты фоновых задач (scheduler), внешних сервисов и других ' +
  'проектов — только чтение ЭТОГО проекта. ' +
  'Если в контексте даны ИНВАРИАНТЫ — строго соблюдай их: не задавай ' +
  ...
  '"suggestion" — это ОДИН конкретный ответ по умолчанию, готовое значение, которое пользователь ' +
  'может принять нажатием Enter; это НЕ список вариантов и НЕ переформулированный вопрос. ' +
  'Пример хорошо: {"question":"Нужна ли онлайн-оплата заказа?","suggestion":"да, картой"}. ' +
  'Пример плохо: {"question":"Какова цель лендинга?","suggestion":"например, привлечение клиентов, ' +
  'продвижение или брендинг"} — здесь в "suggestion" список, а не готовый ответ. ' +
...
export class RunController {
  ...
  private async gatherRequirements(
    ...
    // рабочую директорию/доступ к файлам (это уже определено; правки идут в копию проекта).
    const project = this.deps.projectContext?.() ?? '';
    // Инварианты — первыми: аналитик не должен предлагать нарушающие их варианты.
    const prefix =
      this.invariantsBlock() + (project ? `${project}\n\n` : '') + (context ? `${context}\n\n` : '');
    const issuesBlock =
      issues.length > 0
