...
describe('интеграция: директива RAG подмешивается при подключённом search_docs', () => {
  ...
  it('воспоминание с ответом в истории → дословный повтор, без форс-поиска и гейта', async t => {
    ...
    const session = {
      ...makeSession(),
      ragSources: ['/docs'],
      // История содержит прошлый обмен — recall воспроизводит именно ответ ассистента.
      messages: [
        { role: 'system' as const, content: 'sys' },
        { role: 'user' as const, content: 'каким флагом передать каталог?' },
  ...
  it('grounded-ответ синтезируется на НИЗКОЙ температуре (0.2 по умолчанию)', async t => {
    const temps: (number | undefined)[] = [];
    const client = clientWith(t, async (messages, options) => {
      const head = messages[0]?.content ?? '';
      if (head.includes('воспоминание')) return { content: RECALL_SENTINEL, usage: undefined };
      temps.push(options.temperature); // температура ответа-синтеза
      return { content: validGrounded, usage: undefined };
    });
    const mcp = {
      toolSet: new McpToolSet(groundedConnect),
    ...
    const { finished } = driveInteractive(
      client,
      ['какие профили сканирования есть?', '/exit'],
      0.7, // сессионная температура высокая — grounded-ответ ДОЛЖЕН её игнорировать
      undefined,
      true,
      null,
      { ...makeSession(), ragSources: ['/docs'] },
    ...
    await finished;
    assert.deepEqual(temps, [0.2]); // ответ пошёл на 0.2, а не на сессионных 0.7
  });

  it('grounded, но search_docs недоступен (rag отвалился) → безопасное сообщение, не галлюцинация', async t => {
    // Сервер без search_docs — форс-поиск не сможет обратиться к базе (forcedRagSearch → []).
    const connect: ConnectFn = async name => ({
      name,
      tools: () => [{ name: 'other_tool', description: '', parameters: { type: 'object' } }],
      call: async () => 'x',
      close: async () => {},
    });
    // Модель «готова» галлюцинировать — но до неё дело НЕ дойдёт (guardrail короткозамкнёт).
    const client = clientWith(t, async () => ({
      content: 'ГАЛЛЮЦИНАЦИЯ: профили default/full/quick',
      usage: undefined,
    }));
    const mcp = { toolSet: new McpToolSet(connect), store: memoryStore(new Map([['srv', STDIO]])) };
    const { finished, text } = driveInteractive(
    ...
    await finished;
    const out = text();
    assert.match(out, /Не удалось обратиться к базе знаний/); // безопасное сообщение
    assert.doesNotMatch(out, /ГАЛЛЮЦИНАЦИЯ/); // сырой ответ модели НЕ показан
  });
