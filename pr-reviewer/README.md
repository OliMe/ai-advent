...
каждый пуш. Чтобы не плодить дубли, свои комментарии помечаются скрытым маркером
(`<!-- ai-review -->`), и перед публикацией нового ревью бот **снимает свои прежние** комментарии (по
маркеру) и постит актуальный набор. Так набор комментариев бота всегда отражает текущий diff.
Сопоставление «та же строка» НЕ используется — оно хрупко (модель приписывает тот же дефект к разной
строке между прогонами, а GitHub пересчитывает позиции при изменении ханка). Чужие комментарии не
трогаются. Компромисс: снятие сбрасывает состояние «Resolve conversation» на комментариях бота.
