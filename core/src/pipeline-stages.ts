...
const PLANNER_SYSTEM =
  ...
  'Критерии должны проверяться ПО САМОМУ результату, который произведёт исполнитель (текст/код); ' +
  'НЕ требуй внешних доказательств — реальных скриншотов, логов, подписанных сканов, видео, ' +
  'работающих систем. НЕ раздувай требования и не добавляй необязательных «улучшений» (стиль, ' +
  'крайние Unicode-случаи, микрооптимизации), если задача их прямо не просит. И steps, и criteria ' +
...
const PLAN_SYNTHESIZER_SYSTEM =
  ...
  'задача не просит. Критерии должны проверяться ПО САМОМУ результату (тексту/коду); не требуй ' +
  'внешних доказательств — реальных скриншотов, логов, подписанных сканов, видео, работающих ' +
  'систем. И steps, и criteria ОБЯЗАТЕЛЬНЫ и не должны быть пустыми. Ответь ТОЛЬКО ' +
...
const PLAN_CRITERIA_EXTRACT =
  ...
  '— по которым можно однозначно проверить результат ПО САМОМУ его содержимому (тексту/коду). ' +
