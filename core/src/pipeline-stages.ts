...
const PLANNER_SYSTEM =
  ...
  'систем. Изменения применяет СИСТЕМА сама (без git-веток/коммитов), а проверка запускает только ' +
  'ОБНАРУЖЕННЫЕ команды проекта (тесты/сборка/линтер) — НЕ включай критерии про создание git-веток/' +
  'коммитов, `npm audit` или иные команды, которых проверка не запускает; только проверяемое по ' +
  'файлам и этими командами. План исполняется этапом ВЫПОЛНЕНИЯ — правкой файлов ЭТОГО проекта и ' +
  'запуском его команд; планируй ТОЛЬКО такие шаги для ЭТОЙ задачи. НЕ планируй фоновые задачи ' +
  'планировщика (scheduler), сбор данных/изображений из внешних сервисов, обращения к другим проектам ' +
  'и прочее, не относящееся к задаче и этому проекту. «Не раздувать» относится только к НЕзаявленным «улучшениям» (стиль, крайние ' +
  'Unicode-случаи, микрооптимизации) — явные требования пользователя отбрасывать нельзя. И steps, и criteria ' +
  'ОБЯЗАТЕЛЬНЫ и не должны быть пустыми. Для плана НЕ вычитывай весь исходный код: опирайся на ' +
  'структуру проекта, package.json и конфиги; читай ТОЛЬКО ключевые файлы, детальное чтение и ' +
  'правку кода делает этап ВЫПОЛНЕНИЯ. Ответь ТОЛЬКО объектом JSON (без обрамляющего текста): ' +
...
const EXECUTOR_FILE_SYSTEM =
  ...
  'по коду, посмотреть содержимое каталога, а также СОЗДАТЬ, ИЗМЕНИТЬ и УДАЛИТЬ файл (delete_file — ' +
  'для устаревших или регенерируемых файлов, напр. lock-файла; НЕ затирай файл пустышкой вместо удаления). Выполни задачу СТРОГО ' +
  'ПО ПЛАНУ и критериям, ВНОСЯ РЕАЛЬНЫЕ правки инструментами. КРИТИЧНО: план говорит, ЧТО менять — ' +
  'меняй ТОЛЬКО файлы, которые план прямо требует, и вноси ТОЛЬКО предписанные планом правки; сам ' +
  'решаешь лишь КАК технически их применить (каким инструментом). НЕ трогай, НЕ переписывай и НЕ ' +
  'рефактори файлы, которых план не касается. Если задача — про документацию, правь документацию ' +
  '(README и пр.), а НЕ исходный код. Если план (или его вывод) — что менять нечего, НЕ выдумывай ' +
  'изменений: не делай правок вообще. Применяй изменения инструментами, а НЕ ' +
  'описывай их словами и не возвращай код в ответе. **Для правки существующего файла предпочитай ' +
  'edit_file (точечная замена old_string→new_string) вместо write_file (полная перезапись): меняется ' +
  'только нужный фрагмент — экономнее и не заденешь остальное. write_file — для НОВЫХ файлов или полной ' +
  'пересборки.** Пути указывай ОТНОСИТЕЛЬНО корня проекта. Раскладка файлов проекта дана ' +
  'ниже — НЕ трать вызовы на разведку структуры. Нужные файлы читай ПАЧКОЙ за один вызов read_files ' +
  '(эффективнее многих read_file), затем внеси правки. Если доступен инструмент запуска скриптов проекта ' +
  '(run_command) — ПОСЛЕ правок проверь их им (напр. форматтер/линтер/тесты) и исправь замечания ' +
  'ДО завершения (напр. запусти форматирование или приведи файл к требуемому стилю). Для задач ' +
  'установки/ОБНОВЛЕНИЯ ЗАВИСИМОСТЕЙ используй инструмент run_package_command (напр. ' +
  '`npx npm-check-updates -u`, затем `npm install`; либо `npm update`) — правь версии реальными ' +
  'командами менеджера, а НЕ вручную; после этого прогони проверки. Когда всё ' +
  'сделано — верни КРАТКОЕ резюме: какие файлы создал/изменил и что именно. Резюме — обычным ' +
  'текстом, без JSON и без markdown-ограждений.';
...
function plannerRoleSystem(role: AgentRole): string {
  return (
    ...
    'и не выходи за рамки своей роли — общий план соберёт ведущий планировщик. У тебя есть ' +
    'инструменты чтения проекта (файлы/структура/поиск по коду) — при необходимости изучи ' +
    'ключевые файлы САМ, а не проси об этом. Для плана НЕ ' +
    'вычитывай весь исходный код: опирайся на структуру проекта, package.json и конфиги; читай ' +
    'ТОЛЬКО ключевые файлы, если без них не спланировать — детальное чтение и правку кода делает ' +
    'этап ВЫПОЛНЕНИЯ, не ты. Планируй ТОЛЬКО правку файлов ЭТОГО проекта; НЕ предлагай фоновые ' +
    'задачи (scheduler), сбор данных из внешних сервисов и обращения к другим проектам. ' +
    'Ответь кратко по делу (можно списком); JSON не требуется.'
  );
}

/** Персона синтезатора: свести предложения экспертов в один детальный, но соразмерный план. */
const PLAN_SYNTHESIZER_SYSTEM =
  'Ты — ведущий планировщик. Предложения экспертов разных ролей даны НИЖЕ, в этом же сообщении ' +
  '(раздел «Предложения экспертов»). НЕ запрашивай их у пользователя и НЕ жди никакого ввода — ' +
  'у тебя ВСЁ есть, своди их в план СЕЙЧАС. Сведи их в ОДИН ' +
  ...
  'реальных скриншотов, логов, подписанных сканов, видео, работающих систем. Изменения применяет ' +
  'СИСТЕМА сама (без git-веток/коммитов), а проверка запускает только ОБНАРУЖЕННЫЕ команды проекта ' +
  '(тесты/сборка/линтер) — НЕ включай критерии про git-ветки/коммиты, `npm audit` или иные команды, ' +
  'которых проверка не запускает. План исполняется правкой файлов ЭТОГО проекта и запуском его команд; ' +
  'своди вклады в шаги ТОЛЬКО такого рода для ЭТОЙ задачи. НЕ включай фоновые задачи планировщика ' +
  '(scheduler), сбор данных/изображений из внешних сервисов и прочее вне задачи/проекта. И steps, и criteria ' +
...
const PLAN_CRITERIA_EXTRACT =
  ...
  'Не требуй внешних доказательств (скриншотов, логов, сканов, видео, работающих систем). Изменения ' +
  'применяет СИСТЕМА сама (без git-веток/коммитов), а проверка запускает только ОБНАРУЖЕННЫЕ команды ' +
  'проекта (тесты/сборка/линтер) — НЕ включай критерии про git-ветки/коммиты, `npm audit` или иные ' +
  'команды, которых проверка не запускает. Ответь ТОЛЬКО объектом JSON: {"criteria":[...]}.';
...
async function planWithTeam(
  ...
  const contributions = await runRoleExperts({
    ...
    // Инструменты чтения (git/rag) — чтобы эксперты изучали проект сами, а не просили это в тексте.
    tools: ctx.tools,
  });
  ...
  const briefing = contributions
    ...
    .join('\n\n');
  // Синтезатору инструменты НЕ даём: эксперты уже всё изучили, его дело — свести их вклады. С
  // инструментами он уходил в агентный цикл и вместо плана просил «пришлите предложения» (DeepSeek).
...
/** Максимум файлов в карте раскладки для исполнителя (бережёт контекст на больших репозиториях). */
const FILE_MAP_LIMIT = 400;

/**
 * Блок «Файлы проекта» для промпта исполнителя — список путей (усечён лимитом), чтобы агент не
 * тратил раунды на разведку структуры (list_dir) и читал нужные файлы пачкой. Нет файлов — пусто.
 */
async function fileListingBlock(workspace: FileWorkspace): Promise<string> {
  const files = await workspace.listFiles();
  if (files.length === 0) {
    return '';
  }
  const note =
    files.length > FILE_MAP_LIMIT ? `\n…(показаны первые ${FILE_MAP_LIMIT} из ${files.length})` : '';
  return `\n\nФайлы проекта (читай нужные пачкой через read_files):\n${files.slice(0, FILE_MAP_LIMIT).join('\n')}${note}`;
}
...
async function runFileExecution(
  ...
  const { planBody, criteria } = executionPlanBody(ctx);
  // Карта файлов проекта в контекст — чтобы агент не тратил раунды на разведку структуры (list_dir).
  const fileMap = await fileListingBlock(workspace);
  ...
  const revision =
    issues.length > 0 || ctx.run.correction
      ? '\n\nЭто ДОРАБОТКА: файлы уже содержат твой прошлый результат — устрани замечания, правя ' +
        ...
        (ctx.run.correction ? `\n\nПравка пользователя: ${ctx.run.correction}` : '')
      : '';
  // План — ПЕРВЫМ, с ЧЁТКОЙ меткой «ПЛАН:» (иначе модель не распознаёт шаги как план и просит его
  // прислать) и требованием строго следовать; карта файлов — справочно в конце (иначе модель
  // цепляется за список файлов и правит не то).
  const prompt =
    `${projectPrefix(ctx)}${memoryPrefix(ctx)}Задача: ${ctx.run.title}${requirementsBlock(ctx)}` +
    `\n\nПЛАН (ниже — сам план; выполняй СТРОГО по нему, файлы вне плана НЕ трогай):\n${planBody}` +
    `\n\nКРИТЕРИИ ПРИЁМКИ:\n${criteria}${revision}${fileMap}`;
