  ...
  const revision =
    issues.length > 0 || ctx.run.correction
      ? '\n\nЭто ДОРАБОТКА: файлы уже содержат твой прошлый результат — устрани замечания, правя ' +
        ...
        (ctx.run.correction ? `\n\nПравка пользователя: ${ctx.run.correction}` : '')
      : '';
  // План — ПЕРВЫМ и обрамлён требованием строго следовать ему; карта файлов — справочно в конце
  // (иначе модель цепляется за список файлов и начинает править не то).
  const prompt =
    `${projectPrefix(ctx)}${memoryPrefix(ctx)}Задача: ${ctx.run.title}${requirementsBlock(ctx)}` +
    `\n\nВыполни СТРОГО этот план — делай ТОЛЬКО предписанное им, файлы вне плана не трогай:\n${planBody}` +
