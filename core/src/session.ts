...
export interface Session {
  ...
  ragSources?: string[];
  /**
   * Корни привязанных проектов (День 31): рабочее пространство ассистента разработчика. Список, а
   * не одно значение, — фича часто живёт в нескольких репозиториях сразу (бэкенд + фронтенд).
   * Персистится в файле сессии: привязка переживает перезапуск.
   */
  projects?: string[];
  createdAt: string;
  updatedAt: string;
