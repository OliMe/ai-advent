...
export class MemoryManager {
  ...
  private async extract(newMessages: ChatMessage[]): Promise<CompletionResult> {
    ...
    const instruction =
      ...
      'ВАЖНО: императивные поручения агенту/планировщику на периодическое или отложенное ' +
      'действие («каждые N секунд/минут …», «ежедневно/каждый день в HH:MM …», «напоминай …», ' +
      '«поставь задачу/напоминание …», «проверяй …», «следи за …») — это команды инструментам, ' +
      'а НЕ задача-цель сессии и НЕ инвариант: для них isNewTask=false и invariant="". ' +
      'Без пояснений.\n\nСообщения:\n' +
      dialogue;
