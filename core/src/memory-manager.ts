...
export class MemoryManager {
  ...
  private async extract(newMessages: ChatMessage[]): Promise<CompletionResult> {
    ...
    const instruction =
      ...
      'архитектуру) — они идут в "task". НЕ включай жёсткие правила/запреты проекта — ' +
      'они идут в "invariant", а не в "user" (и не дублируй их в "user"). Бери только из ' +
      'слов пользователя или явных подтверждений; не из предложений ассистента; если ' +
      'таких нет — пустой массив. ' +
      ...
      '"invariant" — жёсткое ограничение, которое нельзя нарушать (архитектура, ' +
      'техрешение, ограничение стека, бизнес-правило). Распознавай его по формулировкам ' +
      'правила: «зафиксируй/зафиксируем», «нельзя», «только», «ни в каких задачах», ' +
      '«всегда», «запрещено», «не использовать X», «использовать только Y». ' +
      'Технологический запрет или выбор стека — это "invariant", а НЕ "user". Дай краткую ' +
      'формулировку; если такого нет — "". ' +
      'Без пояснений.\n\nСообщения:\n' +
      dialogue;
