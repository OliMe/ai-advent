...
export const ORCHESTRATOR_SYSTEM =
  ...
  'экспертов разных ролей, или достаточно одного агента. Собирай команду, если задача ' +
  'охватывает НЕСКОЛЬКО разных аспектов или подсистем, где разные точки зрения дадут более ' +
  'полный и качественный результат (например: архитектура + безопасность + данные + ' +
  'тестирование; или стратегия + контент + процесс + аналитика). Одну роль выбирай ТОЛЬКО для ' +
  'действительно узкой задачи с единственным небольшим результатом — одна функция, короткий ' +
  'скрипт, отдельный маленький модуль, короткая заметка. Не плоди лишних ролей: каждая должна ' +
