...
export const ORCHESTRATOR_SYSTEM =
  ...
  'экспертов разных ролей для максимально полного результата, или достаточно одного ' +
  'универсального агента. Для простой/узкой задачи верни ОДНУ роль. Для сложной — подбери ' +
  'несколько ДОПОЛНЯЮЩИХ ролей (например: архитектор, безопасность, тестирование, UX, данные, ' +
  'производительность, доменный эксперт) — ровно те, что релевантны задаче, и не больше лимита. ' +
  'Каждая роль: краткое имя (name) и focus — что именно она привносит. Если в контексте даны ' +
  'ИНВАРИАНТЫ — учитывай их при подборе ролей. Ответь ТОЛЬКО объектом JSON: ' +
