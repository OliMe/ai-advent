...
export const ORCHESTRATOR_SYSTEM =
  ...
  'экспертов разных ролей, или достаточно одного агента. По умолчанию выбирай ОДНУ роль. ' +
  'Команду из нескольких ролей собирай ТОЛЬКО для действительно крупной, широкой или ' +
  'многогранной задачи (несколько подсистем/аспектов: архитектура + безопасность + данные + …). ' +
  'Узкая или небольшая задача — например, одна функция, скрипт, отдельный модуль, короткий ' +
  'документ — это ВСЕГДА одна роль, даже если ролей можно вообразить много. Лишние роли только ' +
  'раздувают требования и мешают. Для команды подбери несколько ДОПОЛНЯЮЩИХ ролей (например: ' +
  'архитектор, безопасность, тестирование, UX, данные, производительность, доменный эксперт) — ' +
  'ровно те, что релевантны, и не больше лимита. Каждая роль: краткое имя (name) и focus — что ' +
  'именно она привносит. Если в контексте даны ИНВАРИАНТЫ — учитывай их при подборе ролей. ' +
